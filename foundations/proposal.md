...
**Status:** DERIVED. This contract is built on top of the four bedrock foundations ([data-point.md](data-point.md), [collection-program.md](collection-program.md), [pointer.md](pointer.md), [zero-four.md](zero-four.md)) and is revisable without a 4.0 grading event. It is NOT bedrock. The four foundations remain immutable; this file is the wiring under them for one specific 3.0 capability — drafting candidate-1.0 collectors without admitting them to the live floor.
...
A **proposal** is a candidate-1.0 collector (a collection program per [collection-program.md](collection-program.md)) that a 3.0 routine has drafted in response to a *measured gap*, awaiting operator promotion before it joins the live floor as a graduated 1.0.

A proposal is **not** itself a data point — it is authored by a 3.0 process — but every claim about it resolves to a data point. The proposal is wrapped in receipts so the operator's promotion decision rests on structure, not on prose.

A proposal lives under `proposals/{proposal_id}/` and is excluded from the live-floor pointer-resolver registry. Until promoted, no 4.0 bundle, no live 1.0 collector, and no 2.0 signal can depend on it. A 3.0 process may inspect proposals (e.g. to draft a successor), but may not consume their outputs as if they were live measurements.

The proposal mechanism exists to keep `3.0 → new 1.0` honest — i.e., a 3.0 process spawning a new 1.0 collector legitimately, not by free-write. Without it, a 3.0 routine that emits new collectors directly into the live floor is free-writing the bedrock — the anti-pattern [CLAUDE.md](../CLAUDE.md) names as "3.0 free-writing outside its declared 0.1+0.2 fences." With it, the 3.0 *drafts*, the 2.0 emission-readiness signal *gates* (via the operator-approval signal under 0.2 protocol), and the operator *signs*. The leash stays on at the boundary that matters most.
...
| `pre_verification.json` | Structured output of running [collection-program.md](collection-program.md)'s validators against `candidate/` at draft-time. Produced by the 3.0 routine before the operator sees the proposal, so the operator never reviews a candidate that already structurally failed. |
...
| `proposed_by` | pointer | yes | `kind=collector` or `kind=orchestration`. The 3.0 routine (or the orchestration step) that produced this proposal. |
...
Promotion is a 2.0 signal consultation (under 0.2 protocol), per [zero-four.md](zero-four.md) §"2.0 → 3.0 (the 0.3 protocol's gate)".

The signal is the **operator-approval signal** (a 2.0 program under 0.2 protocol):
...
**The leash stays on.** The signal cannot return `promote` without a corresponding operator decision record on disk. The 3.0 routine never auto-promotes; it never writes to `approvals/decisions.jsonl`; it never moves files out of `proposals/`. Promotion is the operator's signature, registered through the signal.

**On `promote`:** a small, deterministic 1.0 promoter program (separate from any 3.0 routine) moves `candidate/{name}.py` into the appropriate live collector directory, registers the kind in the live registry, re-resolves any pointers that targeted the proposal, and writes the new `decision_record_pointer` into a new `proposal.json` record with `status=promoted`.
...
**On `defer`:** no state change; the proposal remains `proposed`. The 3.0 routine sees `defer` and moves on.
...
2. **Gap-anchored.** Every proposal cites at least one live gap data point. The gap is computed by a live 1.0 gap-collector, not asserted by the 3.0 routine. A request-driven proposal cannot exist; the `gap_pointers` field rejects it.
3. **Promotion-gated.** No proposal becomes a live 1.0 collector without an operator decision record routed through the operator-approval signal. The 3.0 routine has no path to the live floor that bypasses the signal.
...
5. **Bedrock-conforming candidate.** If promoted, the candidate enters the live floor having passed the same Foundation-2 validators a hand-written 1.0 collector would. Promotion does not relax the bedrock; it only crosses the boundary.
...
- **Not a data point.** A proposal is authored by a 3.0 process. Data points are computed by 1.0 collectors under 0.1 protocol. The two do not become each other.
- **Not a live 1.0 collector.** Until promoted, the candidate is on disk under `proposals/` but is invisible to the live registry, the live pointer resolvers, and any 4.0 bundle's verify.py. A 3.0 routine that imports from `proposals/` is consuming pre-bedrock material and is operating outside its fence.
- **Not a request.** The 3.0 routine cannot draft a proposal because the operator asked for one. It drafts a proposal because a 1.0 gap-collector emitted a gap data point. Without a live gap data point, there is nothing for `gap_pointers` to point at, and the manifest fails the schema check.
...
- **Structural pre-check** closes *operator-as-validator-for-typos*. The operator's attention is finite; spending it on candidates that already failed Foundation-2 wastes it. The 3.0 routine's job is to filter to candidates worth the operator's read.
- **Mandatory gap pointers** close *request-driven 1.0*. A proposal with no measured gap is a 3.0 free-write claiming to be a 1.0 draft. The schema rejects it.
...
- **Operator-approval as a 2.0 signal** closes *informal promotion*. Without a typed signal, "the operator approved" becomes a vibe; with one, it is a structured record at a known location, consultable by `verify.py` and by future 2.0 signals.
- **Promoter as separate 1.0 program** closes *write-during-decide*. The signal returns a verdict; a different program acts on it. Decision and effect are two events, not one.

Holding all of these at once is what lets the floor grow under discipline. A 3.0 routine can draft a hundred proposals overnight; the floor only grows by what the operator signs and what passes the bedrock checks. Generation is plentiful; promotion is scarce. That asymmetry is the point.
