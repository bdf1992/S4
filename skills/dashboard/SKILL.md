---
name: dashboard
description: Four views over the same live source — markdown render (at-a-glance tables), snapshot capture (persisted observation of state-at-time), narrate (delta-aware prose vs last snapshot), and html (self-contained browser-viewable page with sparkline + cumulative-flow diagram). Composes every existing artifact in the repo — bedrock primitives, board health, floor-ratio with trend, leash registry, per-skill regime distribution, proposal-flow CFD. Pure 1.0 deterministic; no LLM in the renders. Snapshots persist as derived observations under outputs/run-<sha8>/, deduplicated by content hash.
...
6. **Floor growth — peer consumption** — per-skill peer-importer status from [tools/floor_growth.py](../../tools/floor_growth.py) (graduated / candidate / isolated / no_structure), plus the ranked next-target candidates the same collector emits via `--ranked`. Lateral compounding metric: a skill graduates only when peer 3.0 software consumes it (CLAUDE.md §"four-rung graduation"); this section is the dashboard's view of where that consumption is and isn't happening.
7. **Claim health — markdown pointers** — verdict + live_ratio + dangling-pointer list from a live walk via [skills/claim_audit](../claim_audit/SKILL.md). Every other section above renders prose with markdown links into source; this section is the dashboard's audit of *its own pointer-honesty*. `narrate` shows the live_ratio delta and lists pointers that became dangling or were resolved since the last snapshot.
8. **Proposal flow over time** — Cumulative Flow Diagram (CFD) of the proposal pipeline (proposed → promoted | rejected) as a stacked-area inline SVG. Time-series computed deterministically from `approvals/decisions.jsonl` events plus `prop:YYYY-MM-DD:` datestamps parsed off each `proposals/*/proposal.json::proposal_id`. Three bands bottom-up: rejected, promoted, still-awaiting; the width of the top band at any x is the work-in-progress count. HTML view only (the markdown renders skip it — visualizations are HTML-native).
...
- [skills/claim_audit/](../claim_audit/) — sibling skill imported at runtime by `snapshot.py` to compute the claim-health surface; the dashboard is its first peer importer. Run `python -m skills.claim_audit.orchestrate` to emit the full bundle of dangling-link rows.
