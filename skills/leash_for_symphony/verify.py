"""verify.py — the 0.4 grading walker for the leash-for-symphony bundle.

Sibling of skills/leash_for_hooks/verify.py and
skills/leash_for_slash_commands/verify.py — same shape, different
COLLECTORS/SIGNALS/decision points. Imports the bedrock validators and
shared resolvers/signals from leash_for_hooks (round-3 reuse: copy
deferred until the bedrock-lift refactor described in
SKILL.md §"Levels" v0.3).

Adds a vocal-capture self-check: when the bundle declares vocal=true,
verify confirms the vocal_capture_plan.md was emitted and (if a
captured symphony-events.jsonl is present) that it parses.

Usage:
  python -m skills.leash_for_symphony.verify
  python -m skills.leash_for_symphony.verify outputs/run-XXX
"""
...
from skills.leash_for_symphony.collectors import (symphony_field_decl, symphony_workflow,
                                                  exemplar_bundle_state)
from skills.leash_for_symphony.signals import symphony_permission_posture

COLLECTOR_ID = "verify"
KIND = "bundle_self_check"
VALUE_SCHEMA = {"type": "object", "required": ["component", "role", "status"]}
INPUTS = [
    "skills/leash_for_symphony/collectors/*.py",
    "skills/leash_for_symphony/signals/*.py",
    "skills/leash_for_symphony/orchestrate.py",
    "skills/leash_for_symphony/outputs/*",
]
...
SIGNALS = (symphony_permission_posture, emission_readiness)
...
def _check_orchestration_decision_points() -> list[dict]:
    ...
    vocal_wired = ('"vocal"' in src and "vocal_capture_plan" in src)
    ...
    return [_check("orchestrate.py", "orchestration",
                   "pass" if not (missing or missing_fences) and ok
                   and toggle_wired and vocal_wired else "fail",
                   missing_decisions=missing, missing_fences=missing_fences,
                   toggle_wired=toggle_wired, vocal_wired=vocal_wired,
...
def _check_leash_state() -> list[dict]:
    ...
    vocal_present = "vocal" in state
    vocal_is_bool = isinstance(state.get("vocal"), bool)
    return [_check(ls.FILENAME, "leash_state",
                   "pass" if ok and (not vocal_present or vocal_is_bool) else "fail",
                   state=state, validation_reason=reason or "ok",
                   vocal_present=vocal_present, vocal_is_bool=vocal_is_bool)]
...
def _check_output_bundle(bundle_dir: Path) -> list[dict]:
    ...
    # Vocal-capture self-check: when vocal=true, the plan must exist.
    vocal = bool(manifest.get("vocal", False))
    plan_path = manifest.get("vocal_capture_plan_path")
    if vocal:
        plan_present = bool(plan_path) and (bundle_dir / plan_path).exists()
        out.append(_check(bundle_dir.name, "output_bundle.vocal_capture_plan",
                          "pass" if plan_present else "fail",
                          vocal=True, plan_path=plan_path,
                          plan_present=plan_present))
        # If a captured event stream exists, confirm it parses as JSONL.
        events = bundle_dir / "symphony-events.jsonl"
        if events.exists():
            try:
                rows = [json.loads(line) for line in
                        events.read_text(encoding="utf-8").splitlines() if line.strip()]
                out.append(_check(bundle_dir.name, "output_bundle.vocal_event_stream",
                                  "pass", row_count=len(rows)))
            except json.JSONDecodeError as exc:
                out.append(_check(bundle_dir.name, "output_bundle.vocal_event_stream",
                                  "fail", reason=f"jsonl_parse:{exc}"))
    else:
        out.append(_check(bundle_dir.name, "output_bundle.vocal_capture_plan",
                          "pass" if not plan_path else "fail",
                          vocal=False, plan_path=plan_path,
                          note="plan absent on non-vocal bundle"))
    return out
