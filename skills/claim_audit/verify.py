"""verify.py — the 0.4 grading walker for the claim_audit bundle.

Runs the grading procedure declared in foundations/zero-four.md against
the skill itself (always) and optionally against a specific output
bundle (when given as argv[1]).
...
Usage:
  python -m skills.claim_audit.verify
  python -m skills.claim_audit.verify outputs/run-XXX
"""
...
INPUTS = [
    "skills/claim_audit/collectors/*.py",
    "skills/claim_audit/signals/*.py",
    "skills/claim_audit/lib/*.py",
    "skills/claim_audit/orchestrate.py",
    "skills/claim_audit/outputs/*",
]

SKILL = Path(__file__).resolve().parent
COLLECTORS = (markdown_claims,)
SIGNALS = (claim_health,)
...
def _check_orchestration_decision_points() -> list[dict]:
    ...
    declared = ["dataset_present", "claim_health_fit"]
    fences = ["markdown_claims", "claim_health"]
...
def _check_output_bundle(bundle_dir: Path) -> list[dict]:
    ...
    out.append(_check(bundle_dir.name, "output_bundle.claim_consistency",
                      "pass" if claim in ("healthy", "degraded", "no_data") else "fail",
                      claim=claim))
    return out
