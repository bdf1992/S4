...
A skill whose only job is to measure 3.0 orchestrations from beneath, by walking the artifacts they write while running, and turning them into Foundation-1 data points.

## Why this exists

The orchestration log every leash and audit emits — `skills/*/outputs/run-*/orchestration-log.jsonl` — is **3.0 self-report**, not a 1.0 dataset. An independent 1.0 collector that walks those logs and re-derives the same data points across runs converts self-report into measurement. Without that conversion, any 2.0 signal trained on the logs would be fitting an LLM-adjacent corpus and would invert the bedrock direction (2.0 leaning on 3.0 instead of the reverse).
...
- [collectors/pfsm_parameters.py](collectors/pfsm_parameters.py) — **2.0 model fit (under 0.2).** Deterministic SGD over a feature-bag linear softmax classifier (skill one-hot + decision counts + fence counts + bias), 500 iterations, fixed seed via SHA-256 expansion. Emits one `pfsm_parameters` data point containing fitted weights, classes, hyperparameters, training_runs, and final_loss.
- [signals/trace_conformity.py](signals/trace_conformity.py) — **2.0 signal (under 0.2).** Loads `pfsm_parameters` and exposes `evaluate(trace) -> {verdict, confidence, top_class, class_probabilities, evidence_pointers, gap_record}`. Verdict is `ready` (top class is 4.0 above threshold), `not_ready` (top class is candidate/rejected, or near-uniform softmax), or `inconclusive` (top class is a non-4.0 success class). The `gap_record` reports corpus dimensions on every non-ready emission.
...
- **Threshold calibration for `verdict=ready`** — `READY_THRESHOLD = 0.5` is placeholder; real calibration requires at least one `claim:4.0` trace held out from training.
